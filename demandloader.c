...
{   
	...
	return (int)mmap((void*) addr, PAGE_SIZE, prot, MAP_FIXED | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
}

void *elf_map(Elf64_Addr addr, int prot, int type, int fd, Elf64_Phdr *pp)
{
...
int load_elf_binary(int fd, Elf64_Ehdr *ep, int argc, char *envp[])
{
	...
	for (i = 0; i < ep->e_phnum; i++) {
		...
		if (ph_table[i].p_flags & PF_R) elf_prot |= PROT_READ;
		if (ph_table[i].p_flags & PF_W) elf_prot |= PROT_WRITE;
		if (ph_table[i].p_flags & PF_X) elf_prot |= PROT_EXEC;
...
{	
	...
	if ((pp->p_vaddr <= addr) && (addr <= pp->p_vaddr + pp->p_filesz) ||
		...
		if (elf_map(addr, elf_prot, MAP_PRIVATE | MAP_FIXED | MAP_EXECUTABLE, fd, pp) < 0) {
...
int main(int argc, char *argv[], char *envp[])
{
	...
	if (argc < 2) {
		fprintf(stderr, "./loader exe_file\n");
		exit(EXIT_FAILURE);
	}
